Shader "TeachShader/Lesson68_AttenLightAndShadow"
{
    Properties
    {
    ...
    SubShader
    {
        Pass
        {
            ...
            fixed4 frag(v2f data) : SV_Target
            {
                ...
                // fixed attenuation = 1;
                //
                // fixed3 shadowAttenuation = SHADOW_ATTENUATION(data);

                UNITY_LIGHT_ATTENUATION(atten, data, data.worldVertex);

                fixed3 color = UNITY_LIGHTMODEL_AMBIENT + (lambertDiffuseColor + phongSpecularColor) * atten;
